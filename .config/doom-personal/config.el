...
(defun shalev/year-calendar (&optional year)
  (interactive)
  (require 'calendar)
  (let* (
...
(defun shalev/scroll-year-calendar-forward (&optional arg event)
  ...
  (save-selected-window
    ...
    (unless (zerop arg)
      (let* (
        ...
        (shalev/year-calendar year)))
    (goto-char (point-min))
    (run-hooks 'calendar-move-hook)))

(defun shalev/scroll-year-calendar-backward (&optional arg event)
  "Scroll the yearly calendar by year in a backward direction."
  (interactive (list (prefix-numeric-value current-prefix-arg)
                     last-nonmenu-event))
  (shalev/scroll-year-calendar-forward (- (or arg 1)) event))

(map! :leader
      :desc "Scroll year calendar backward" "<left>" #'shalev/scroll-year-calendar-backward
      :desc "Scroll year calendar forward" "<right>" #'shalev/scroll-year-calendar-forward)

(defalias 'year-calendar 'shalev/year-calendar)
...
(setq doom-font (font-spec :family "JetBrains Mono Nerd Font" :size 20)
      doom-variable-pitch-font (font-spec :family "JetBrains Mono Nerd Font" :size 20)
...
(defun shalev/insert-todays-date (prefix)
  (interactive "P")
  (let ((format (cond
                 ((not prefix) "%A, %B %d, %Y")
...
(require 'calendar)
(defun shalev/insert-any-date (date)
...
(map! :leader
      (:prefix ("i d" . "Insert date")
        :desc "Insert any date"    "a" #'shalev/insert-any-date
        :desc "Insert todays date" "t" #'shalev/insert-todays-date))
...
(set-face-attribute 'mode-line nil :font "JetBrainsMono Nerd Font")
...
(map! :leader
      (:prefix ("=" . "open file")
       :desc "Edit agenda file"      "=" #'(lambda () (interactive) (find-file "~/.dotfiles/.config/doom/start.org"))
...
(map! :leader
      (:prefix ("= e" . "open eshell files")
       :desc "Edit eshell aliases"   "a" #'(lambda () (interactive) (find-file "~/.dotfiles/.config/doom/eshell/aliases"))))

(map! :leader
      :desc "Org babel tangle" "m B" #'org-babel-tangle)
(after! org
  (setq org-directory "~/2-areas/org"
...
(setq eshell-aliases-file "~/.config/doom/eshell/aliases"
      ...
      eshell-visual-commands'("bash" "fish" "top" "htop" "btop" "ssh" "zsh"))
(map! :leader
...
(evil-define-key 'normal start-mode-map
  (kbd "1") '(lambda () (interactive) (find-file "~/.dotfiles/.config/doom/config.org"))
  (kbd "2") '(lambda () (interactive) (find-file "~/.dotfiles/.config/doom/init.el"))
  (kbd "3") '(lambda () (interactive) (find-file "~/.dotfiles/.config/doom/packages.el"))
  (kbd "4") '(lambda () (interactive) (find-file "~/.dotfiles/.config/doom/eshell/aliases")))
