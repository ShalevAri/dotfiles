#+TITLE: Shalev's Doom Emacs Config
#+DESCRIPTION: Shalev's personal Doom Emacs config.
#+AUTHOR: Shalev Ari
...
* ABOUT THIS CONFIG
This is my personal DOOM Emacs config. It is personalized to my needs and is made to behave the way I like it.

* BEACON
Make the cursor shine when scrolling (this is a global minor-mode)
...
* CALENDAR
Let's make a 12-month calendar
...
* CENTAUR-TABS
Enable tabs (be sure to uncomment "tabs" in Doom's init.el)
...
* DOOM THEME
Set the theme to Catppuccin Mocha. To try out new themes, I set a keybinding for counsel-load-theme with 'SPC h t'.
=note=: when setting `catppuccin` as the theme, the `mocha` variant is chosen by default (so we don't need to specify it manually)
...
* LINE SETTINGS
I set comment-line to 'SPC TAB TAB' which is easy and quick to press. The standard Emacs keybinding for comment-line is 'C-x C-;'. The other keybindings are for commands that toggle on/off various line-related settings. Doom Emacs uses 'SPC t' for "toggle" commands, so I choose 'SPC t' plus 'key' for those bindings.
...
* MINIMAP
A minimap sidebar displaying a smaller version of the current buffer on either the left or right side. It highlights the currently shown region and updates its position automatically (does not work on org documents)
...
* MOUSE SUPPORT
Enable mouse support in the terminal version of Emacs (how dare you)
...
** Vterm
I'd like Vterm to open in the current directory by default so I set it here.
...
* START PAGE
Instead of using Doom's Dashboard or the Emacs Dashboard program, I have decided to just set a custom start file as my "dashboard" since it allows me more customization options. I have added to the 'start-mode-hook' the argument 'read-only-mode'. This is to prevent accidental editing of the start file, and to prevent clashes with the 'start-mode' specific keybindings. You can toggle on/off read-only-mode with 'SPC t r'.
